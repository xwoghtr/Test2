...
namespace ConsoleApplication2
{
    ...
    {
        Dictionary<char, double> samo = new Dictionary<char, double>();
        Dictionary<char, double> spol = new Dictionary<char, double>();
        public Generowanie3()
        {
            samo.Add('a', 0.0891);
            spol.Add('ą', 0.0099);
        ...
        {
            Dictionary<char, double> sum = new Dictionary<char, double>();
            foreach (var s in samo)
                if (dozwoloneSamo.Contains(s.Key))
                    sum.Add(s.Key, s.Value);
            foreach (var s in spol)
                if (dozwoloneSpol.Contains(s.Key))
                    sum.Add(s.Key, s.Value);

            double sumprap = sum.Sum(a => a.Value);
            Random rand = new Random();
            double randDouble = rand.NextDouble() * sumprap;
